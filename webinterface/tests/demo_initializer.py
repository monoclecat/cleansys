...
def create_demo_database():
    if Schedule.objects.exists() or ScheduleGroup.objects.exists() or Cleaner.objects.exists():
        logging.error("The database needs to be empty of CleanSys objects in order to create the demo database!")
        return False

    DEMO_LENGTH = 16  # 16 weeks

    logging.info("Creating Schedules...")
    ...
    logging.info("Creating ScheduleGroups...")
    ...
    logging.info("Creating Cleaners...")
    ...
    logging.info("Creating TaskTemplates...")

    def create_task_templates(schedule: Schedule, template_tuples: list):
        for name, help_text, disabled, start, end in bathroom_tasks:
            TaskTemplate.objects.create(task_name=name, task_help_text=help_text, disabled=disabled, schedule=schedule,
    ...
    logging.info("Creating Affiliations...")
    ...
    def affiliate_multiple_cleaners(cleaners: list, group_sequence: list):
        max_pow_of_2 = math.floor(math.log(len(group_sequence), 2))
        for i, cleaner in enumerate(cleaners):
            groups_for_cleaner = group_sequence[:math.pow(2, (i % (max_pow_of_2+1)))]
    ...
    logging.info("Creating Assignments (this can take some time)...")
    ...
    logging.info("Creating DutySwitch objects...")
    # Sprinkle some dutyswitch requests over the cleaning_weeks
    for cl in [cl_a, cl_d, cl_g, cl_j, cl_m]:
        assignments = Assignment.objects.filter(cleaner=cl, cleaning_week__week__range=(now, now+DEMO_LENGTH-3-4))
        for i in range(0, 2):
            choice = random.choice(assignments)
            DutySwitch.objects.create(requester_assignment=choice)
            assignments = assignments.exclude(choice)

    logging.info("Last tweaks...")
    ...
    for task in Task.objects.filter(cleaning_week__week__range=(now-3, now)):
        task.set_cleaned_by(random.choice(task.possible_cleaners()))
    ...
    for i in range(0, 10):
        ...
        cleaned_tasks = cleaned_tasks.exclude(uncleaned_task)
