...
class CleaningDayQuerySetTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.enabled = CleaningWeek.objects.create(date=cls.reference_date1, schedule=cls.schedule, disabled=False)
        cls.disabled = CleaningWeek.objects.create(date=cls.reference_date2, schedule=cls.schedule, disabled=True)
...
class CleaningDayTest(TestCase):
    ...
    def test__creation(self):
        cleaning_day = CleaningWeek.objects.create(date=datetime.date(2010, 2, 8), schedule=self.schedule)
        self.assertIsInstance(cleaning_day, CleaningWeek)
